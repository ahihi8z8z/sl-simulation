...
| `min_instances` | int | không | 0 | Số warm containers tối thiểu. Autoscaler đảm bảo duy trì ít nhất bấy nhiêu warm containers. Idle eviction bỏ qua nếu evict sẽ giảm xuống dưới min. LRU eviction dưới memory pressure CÓ THỂ vi phạm (soft guarantee) |
| `max_instances` | int | không | 0 | Giới hạn tổng containers trên tất cả nodes (0 = unlimited). Khi đạt giới hạn, request mới bị drop với `drop_reason = "max_instances"` |

**Lưu ý:** `prewarm_count`, `idle_timeout`, `pool_targets` **không còn** là cấu hình service. Các tham số này giờ do controller/policy quản lý tại runtime thông qua AutoscalingAPI (`set_pool_target()`, `set_idle_timeout()`). Xem phần [controller](#controller-tùy-chọn-yêu-cầu-autoscaling-enabled) và [API Reference](api_reference.md).
...
Khi `enabled: true`:
- **Reconcile** (periodic): evict warm containers idle quá `idle_timeout` (tôn trọng `min_instances`), containers ở intermediate states không bị idle evict, LRU evict khi overcommit memory (CÓ THỂ vi phạm min_instances)
- **Pool top-up** (reactive): fill lại pool ngay khi instance bị evict, bị promote, warm→running, hoặc target/min_instances thay đổi. Không đợi reconcile. Budget logic: fill warm đến min_instances trước, sau đó pool_targets với budget còn lại (max_instances - total)
- **Initial fill**: fill warm đến min_instances và pools đến target khi startup
- Warm containers tạo tự nhiên bởi request, đảm bảo tối thiểu bởi min_instances, giới hạn tổng bởi max_instances
...
| `cpu_high` | float | 0.8 | Ngưỡng CPU cao → tăng pool_target, giảm idle_timeout |
| `cpu_low` | float | 0.3 | Ngưỡng CPU thấp → giảm pool_target, tăng idle_timeout |
| `pool_target_max` | int | 10 | Giới hạn trên pool_target (backward-compat: `prewarm_max` cũng được chấp nhận) |
| `pool_target_min` | int | 0 | Giới hạn dưới pool_target (backward-compat: `prewarm_min` cũng được chấp nhận) |
| `pool_target_step` | int | 1 | Bước tăng/giảm pool_target (backward-compat: `prewarm_step` cũng được chấp nhận) |
...
**Lưu ý:** Controller/policy quyết định `pool_targets` và `idle_timeout` tại runtime dựa trên metrics. `min_instances` và `max_instances` là cấu hình user ở section `services`, không do controller điều chỉnh.
...
| `pool_target_max` | int | 10 | Giới hạn trên action tăng pool target (backward-compat: `prewarm_max` cũng được chấp nhận) |
