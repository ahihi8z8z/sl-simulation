...
```
Generator  →  LoadBalancer  →  Node Queue  →  Cold Start / Reuse  →  Execution  →  Completion
   |               |               |                |                    |              |
...
**Lưu ý:** Queue là `simpy.Store` (unbounded). Nhiều request có thể nằm trong queue đồng thời.

## Phase 4: Chọn Instance

**File:** `cluster/node.py` — `_process_request()`

### 4a. Kiểm tra timeout trước

```python
remaining = invocation.timeout - (env.now - invocation.arrival_time)
if remaining <= 0:
    # Đã hết timeout từ trước khi bắt đầu xử lý
    _timeout_invocation(invocation)
    return
```

### 4b. Tìm warm instance
...
Nếu không tìm thấy warm instance → tạo mới:
