...
| Method | Signature | Mô tả |
|--------|-----------|-------|
| `get_pool_target` | `(service_id: str, state: str) → int` | Target số container tại state này hoặc sâu hơn |
| `set_pool_target` | `(service_id: str, state: str, count: int) → None` | Đặt target. Có hiệu lực từ reconcile tiếp theo |
...
Pool targets cho phép kiểm soát số container được pre-warm đến từng state trong cold-start chain. Container ở state sâu hơn cũng được tính vào target state nông hơn.

Ví dụ với chain `[null, prewarm, code_loaded, warm]`:

```
pool_target("svc-a", "prewarm")     = 3  → giữ 3 container tại prewarm+
pool_target("svc-a", "code_loaded") = 2  → giữ 2 container tại code_loaded+
pool_target("svc-a", "warm")        = 1  → giữ 1 container tại warm
```
