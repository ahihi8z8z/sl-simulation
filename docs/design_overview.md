...
| `transition_model.py` | `TransitionModel` — pluggable model quyết định time/cpu/memory cho mỗi transition. 3 implementation: `DeterministicTransitionModel` (mặc định, giá trị cố định từ config), `CsvSampleTransitionModel` (sample từ CSV trace), `DistributionTransitionModel` (lognormal/normal/uniform) |
...
**TransitionModel pluggable:** Giá trị `time`, `cpu`, `memory` cho mỗi transition được cung cấp bởi `TransitionModel`. Mặc định dùng `DeterministicTransitionModel` (giá trị cố định từ config). Khi cấu hình `transition_profile` trỏ đến file CSV, hệ thống dùng `CsvSampleTransitionModel` — sample ngẫu nhiên từ dữ liệu thực tế, cho phép mô phỏng biến thiên cold start time.
