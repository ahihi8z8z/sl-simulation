...
| File | Chức năng |
|------|-----------|
| `service_class.py` | `ServiceClass` dataclass — định nghĩa service: arrival_rate, job_size, timeout, memory, cpu, max_concurrency |
...
| File | Chức năng |
|------|-----------|
| `load_balancer.py` | `ShardingContainerPoolBalancer` — consistent hash ring. Hash `service_id` để chọn primary node, walk ring tìm node có đủ memory |

**Thuật toán dispatch:**
1. Hash `md5(service_id)` → primary node index
2. Thử primary node: kiểm tra `node.available.memory >= service.memory`
3. Nếu không đủ: walk vòng ring (offset 1, 2, ..., n-1)
4. Nếu không node nào đủ: DROP request với `drop_reason="no_capacity"`
...
| `state_machine.py` | `OpenWhiskExtendedStateMachine` — đồ thị trạng thái với BFS pathfinding. Hỗ trợ default và config-driven |
| `lifecycle_manager.py` | `LifecycleManager` — find/reuse instance, cold start, start/finish execution, eviction |

**Default state machine:**
```
null --0.5s--> prewarm --0.3s--> warm --0s--> running --0s--> warm --0s--> evicted
                  |                                                          ^
                  +----------------------------------------------------------+
```

**Extended state machine (config-driven):** Có thể thêm states trung gian như `code_loaded`, `deps_loaded` giữa `prewarm` và `warm`, mỗi state có transition time và resource cost riêng.

### autoscaling/ — Tự động scale

| File | Chức năng |
|------|-----------|
| `autoscaler.py` | `OpenWhiskPoolAutoscaler` — 3 giai đoạn mỗi reconcile cycle: (1) evict idle quá timeout, (2) LRU evict khi hết memory, (3) top-up prewarm containers |
| `autoscaling_api.py` | `AutoscalingAPI` — facade để controller/RL điều chỉnh prewarm_count và idle_timeout |
...
| `policies/threshold_policy.py` | `ThresholdPolicy` — rule-based: CPU cao → tăng prewarm, giảm idle_timeout; CPU thấp → ngược lại |
...
| `summary_writer.py` | `SummaryWriter` — ghi `summary.txt` với thống kê request và latency |
...
2. **Central request table:** `ctx.request_table` là dict duy nhất chứa tất cả Invocation. Mọi component ghi trạng thái vào đây — giúp export và monitoring truy vấn nhất quán.
