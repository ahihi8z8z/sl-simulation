...
| File | Chức năng |
|------|-----------|
| `autoscaler.py` | `OpenWhiskPoolAutoscaler` — 3 giai đoạn mỗi reconcile cycle: (1) evict idle quá idle_timeout, (2) LRU evict khi hết memory, (3) top-up pool targets cho các state trung gian |
| `autoscaling_api.py` | `AutoscalingAPI` — facade để controller/RL điều chỉnh pool_targets (per-state), idle_timeout. Warm containers không có target — tạo tự nhiên bởi request, giới hạn bởi memory |
