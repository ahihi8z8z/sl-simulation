...
| File | Chức năng |
|------|-----------|
| `service_class.py` | `ServiceClass` dataclass — định nghĩa service: arrival_rate, job_size, memory, cpu, max_concurrency, pool_targets |
...
| File | Chức năng |
|------|-----------|
| `autoscaler.py` | `OpenWhiskPoolAutoscaler` — reconcile chỉ xử lý eviction (idle timeout + LRU). Pool top-up là **reactive**: trigger ngay khi instance bị evict, bị promote, hoặc khi target thay đổi. `initial_fill()` fill pool khi startup |
