...
**Mặc định:** `Box(shape=(11,), dtype=float32)` với 11 metrics:
...
| 4 | `request.timed_out` | Tổng số request bị timeout |
| 5 | `request.in_flight` | Số request đang xử lý |
| 6 | `request.cold_starts` | Tổng số cold starts |
| 7 | `lifecycle.instances_total` | Tổng container (không kể evicted) |
| 8 | `lifecycle.instances_warm` | Container đang warm (idle, sẵn sàng) |
| 9 | `lifecycle.instances_running` | Container đang chạy request |
...
| 1 | Tăng `prewarm_count` thêm 1 (tối đa `prewarm_max`) |
| 2 | Giảm `prewarm_count` đi 1 (tối thiểu 0) |
...
| `timeout_penalty` | -1.0 | Phạt cho mỗi request bị timeout trong step |
...
**Lưu ý:** `d_completed` là số request mới hoàn thành trong step này (không phải tổng). Tương tự cho d_dropped, d_timed_out, d_cold_starts.
...
2. Gọi `AutoscalingAPI` để thay đổi prewarm_count hoặc idle_timeout
3. `env.run(until=env.now + step_duration)` — chạy thêm N giây simulation
