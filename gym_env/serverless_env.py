...
class ServerlessEnv(gym.Env):
    ...
    Config (gym_config_path JSON):
        ...
        control_idle_timeout     : bool  (default True; False → drop idle dims)
        control_pool_target      : bool  (default True; False → drop pool dims)
    ...
    def _build(self) -> None:
        ...
        # Warmup phase: run the autoscaler with NO traffic so it can fill the
        # initial pool. Then start workload. Same order as sim_engine — keeps
        # gym infer comparable to standalone sim runs.
        ...
        self._action_mapper = MultiActionMapper(
            ...
            control_pool_target=self.gym_config.get("control_pool_target", True),
